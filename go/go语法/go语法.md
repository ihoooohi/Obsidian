...
**面试回答**
桶号是ash 的低 B 位，Go map 扩容时，桶数翻倍，通过多看 hash 的一位来决定元素是否迁移，不需要重新计算 hash，迁移成本很低。
...
**面试回答**
原来冲突严重，是因为旧的 hash 分布不好；  
换一个 seed，相当于换一套“洗牌规则”，让 key 重新随机分布。
...
```
但是如果这样，然后使用时传入u User就不行，因为编译器不知道v 是什么？有多少字段？字段叫什么？
